...
import { htmlCourseData } from "../data/htmlCourse.js"; // ← FIXED: добавлено .js

export default function HTMLCoursePage({ onBack }) {
  const theme = useTheme();
  const completedTopics = htmlCourseData.topics.filter(topic => topic.completed).length;
  const totalTopics = htmlCourseData.topics.length;
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
          ...
          <Box>
            <Typography variant="h4" fontWeight="bold">
              {htmlCourseData.title}
            </Typography>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: 1 }}>
              <Chip
                label={htmlCourseData.level}
                size="small"
                sx={{
      ...
      <Box sx={{ maxWidth: "100%", mx: "auto" }}>
        ...
        {htmlCourseData.topics.map((topic) => (
